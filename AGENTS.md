...
- **whisper-server needs ffmpeg at runtime.** The `--convert` flag shells out to `ffmpeg` for audio transcoding. Missing ffmpeg manifests as `logs/whisper-server_port_9000.log` containing only `ffmpeg is not found.` and the process *not* exiting — it sits running but never binds to port 9000, so health probes time out. `setup-whispercpp.sh` installs ffmpeg up front via the detected package manager before the build step; do not gate that on the build (the binary can already exist from a prior run while ffmpeg is still missing).
- **Health probe retries must catch `httpx.ConnectTimeout`, not just `ConnectError`.** When a local server is launched but hasn't bound its port (e.g. whisper-server stuck on missing ffmpeg), httpx raises `ConnectTimeout` rather than `ConnectError`. `ServerManager._wait_for_health` catches both so the retry loop continues until `STARTUP_TIMEOUT`, then fails gracefully with a log tail via `display.server_timeout` / `server_failed`. Letting `ConnectTimeout` escape kills the `switch` / `pipeline` worker entirely and the user sees a full traceback instead of a nice rollback.
