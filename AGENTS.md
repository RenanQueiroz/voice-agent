...
- **Whisper model name.** In local STT, `ModelConfig.model` is a whisper.cpp model file suffix like `large-v3-turbo-q8_0` (matches `ggml-{name}.bin`), *not* an MLX path. `setup-whispercpp.sh` manages these files.
