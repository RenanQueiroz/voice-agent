...
- **LLM server** (port 8080) command depends on the active LLM's `runtime` field (`mlx-vlm` / `llamacpp`); swapping between local LLMs restarts. The health path comes from the `RUNTIMES` registry (`/v1/models` for mlx-vlm, `/health` for llamacpp).
