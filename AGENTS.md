...
- **whisper-server** (STT port 9000) takes `-m <model>` at startup — swapping local STT models restarts. It should not be launched with whisper.cpp server-side VAD or `--convert`: `VADRecorder` already produces speech segments, and `WhisperCppSTTModel` uploads mono 16 kHz WAV directly.
...
- **Local whisper STT is already post-VAD.** Do not re-enable whisper.cpp server-side `--vad` or `--convert` unless the client stops sending direct WAV. The current request sets `response_format=text`, `no_timestamps=true`, and `temperature_inc=0.0` because the app only needs text and interactive latency matters more than JSON metadata or fallback retries.
