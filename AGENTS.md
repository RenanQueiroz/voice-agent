...
Real-time speech-to-speech voice agent built on the OpenAI Agents SDK's `VoicePipeline`. The UI is a fullscreen Textual app. Each role (STT / LLM / TTS) is configured independently as either cloud (OpenAI or Gemini) or local (whisper.cpp / mlx-audio / mlx-vlm / mlx-lm / llama-server / supertonic / kokoro-fastapi / qwen3-tts); the user can mix-and-match and swap at runtime.

Runs on macOS (whisper.cpp + llama.cpp + mlx-* + supertonic + any cloud role) and Linux (llama.cpp + whisper.cpp + supertonic CPU TTS + kokoro-fastapi/qwen3-tts CUDA TTS + any cloud role — the mlx stack is Apple-Silicon-only and is filtered out of the catalog on Linux). See the runtime registry below.
