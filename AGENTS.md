...
- **LLM server** (port 8080) command depends on the active LLM's `runtime` field (`mlx-vlm` / `llamacpp`); swapping between local LLMs restarts. The health path comes from the `RUNTIMES` registry (`/v1/models` for mlx-vlm, `/health` for llamacpp). The app starts llama.cpp with `--models-preset <preset> --models-max 1` so the router never keeps more than one model loaded for app-managed runs. `./start-llamacpp.sh` is the standalone path for using llama.cpp without the Textual app: it ensures `./llamacpp/llama-server` exists, then execs it with `--host 0.0.0.0 --port 8080 --models-preset ./llamacpp-models.ini --models-max 1` by default, while forwarding extra CLI args after those defaults so standalone callers can intentionally override options such as `--models-max`. Keep its defaults in sync with this app launcher unless there is a deliberate compatibility reason to diverge. `./bench.sh` is the standalone llama-bench helper: it parses `llamacpp-models.ini`, lets the user choose a model section with arrow keys + Enter, maps only supported preset fields to `llama-bench`, and writes CSV output under `logs/`.
