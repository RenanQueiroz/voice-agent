...
Real-time speech-to-speech voice agent built on the OpenAI Agents SDK's `VoicePipeline`. The UI is a fullscreen Textual app. Each role (STT / LLM / TTS) is configured independently as either cloud (OpenAI or Gemini) or local (whisper.cpp / mlx-audio / mlx-vlm / llama-server / supertonic / kokoro-fastapi / qwen3-tts); the user can mix-and-match and swap at runtime.

Runs on macOS (whisper.cpp + llama.cpp + mlx-vlm + mlx-audio + supertonic + any cloud role) and Linux (llama.cpp + whisper.cpp + supertonic CPU TTS + kokoro-fastapi/qwen3-tts CUDA TTS + any cloud role — the mlx stack is Apple-Silicon-only and is filtered out of the catalog on Linux). See the runtime registry below.
...
- **Local whisper STT is already post-VAD.** Do not re-enable whisper.cpp server-side `--vad` or `--convert` unless the client stops sending direct WAV. The current request sets `response_format=text`, `no_timestamps=true`, `temperature_inc=0.0`, and the active STT entry's `language` because the app only needs text and interactive latency matters more than JSON metadata or fallback retries. For local whisper.cpp, omitted `language` defaults to `"auto"`.
