...
- **qwen3-tts also needs Python dev headers at runtime** (apt: `python3.12-dev`, dnf/zypper: `python3-devel`; Arch ships them in the main `python` package). torch.compile / Triton shells out to gcc per compiled graph to build a tiny `cuda_utils.so` whose C source does `#include <Python.h>`. Without the headers the compile fails with `BackendCompilerFailed: CalledProcessError ... 'gcc' ... 'Python.h': No such file or directory` — NOT at server startup (the /health probe passes fine), but mid-audio-generation on the first TTS request. The setup script installs the dev package and `venv_has_python_headers` gates the fast-path + post-install verification so a missing header invalidates the install rather than caching a broken state.
- **qwen3-tts install order matters: `-e ".[api]"` MUST run BEFORE the cu121 torch pin, and the flash-attn install MUST use `--no-deps`.** Two independent footguns both silently skew torch back to PyPI's latest:
  1. Qwen3-TTS's pyproject leaves torch unpinned, so `uv pip install -e ".[api]"` upgrades whatever torch is in the venv to the latest PyPI wheel. Install `.[api]` first, then `uv pip install --reinstall torch torchaudio --index-url ...cu121` to force-replace it.
  2. flash-attn's pyproject lists torch as a runtime dependency. Without `--no-deps`, `uv pip install flash-attn --no-build-isolation` re-resolves all flash-attn deps — including torch — against the default PyPI index, clobbering the cu121 pin a second time. The flash-attn install step must pass `--no-deps`; torch is already in the venv at that point, there's nothing to re-resolve.
  Symptom of either skew: `libtorchaudio.so` fails to dlopen (ABI mismatch with torch), the backend logs "Backend initialization delayed" and defers to "on first request", `/health` returns 200 with `status=initializing` forever, splash hangs. The setup script's final verification step imports `torch + torchaudio + flash_attn` + checks `torch.version.cuda.startswith("12.1")` BEFORE writing the stamp — so a regression to either footgun fails the install rather than caching the broken venv.
